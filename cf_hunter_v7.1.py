...
╚██████╗██║           ██║  ██║╚██████╔╝██║ ╚████║   ██║   ███████╗██║  ██║
 ╚═════╝╚═╝           ╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═══╝   ╚═╝   ╚══════╝╚═╝  ╚═╝
{C.RESET}{C.DIM}  CF-HUNTER v7.1 SAFE — Unified Recon + Verify + Full Audit ++{C.RESET}
""")
...
_HEADERS = {
    "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) CF-Hunter/7.1-safe",
...
def build_text_summary(domain, posture, anomalies, origin_candidates):
    ...
    if origin_candidates:
        top = origin_candidates[0][1]
        lines.append(f"Ən güclü origin namizədi: {origin_candidates[0][0]} | role={top.get('role')} | score={top.get('confidence_score')}")
    else:
...
def build_markdown_report(result):
    ...
    lines = []
    lines.append(f"# CF-HUNTER v7.1 SAFE report")
    lines.append("")
...
def analyze(domain, output_file=None, verbose=False, skip_github=False, skip_verify=False,
    ...
    if write_md:
        md_path = output_file.rsplit(".", 1)[0] + ".md" if output_file else f"{domain}_cfhunter_v7_1.md"
...
def main():
    banner()
    ap = argparse.ArgumentParser(description="CF-Hunter v7.1: Unified safe Cloudflare origin intelligence, posture auditor and analyst report engine")

...
╚██████╗██║           ██║  ██║╚██████╔╝██║ ╚████║   ██║   ███████╗██║  ██║
 ╚═════╝╚═╝           ╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═══╝   ╚═╝   ╚══════╝╚═╝  ╚═╝
{C.RESET}{C.DIM}  CF-HUNTER v7.2.1 SAFE — Adaptive, Faster, Analyst-Grade{C.RESET}
""")
...
def build_markdown_report(result):
    ...
    lines = []
    lines.append(f"# CF-HUNTER v7.2.1 SAFE report")
    lines.append("")
...
# Main analyze
# ──────────────────────────────────────────────────────────────
def _analyze_core(domain, output_file=None, verbose=False, skip_github=False, skip_verify=False,
...
def _family_key(name: str) -> str:
    ...
    for k in PRIORITY_SUBDOMAIN_HINTS:
        if k in host:
            return k
    if host.startswith('www'):
        return 'www'
    return host[:12] or 'other'


def cluster_subdomain_families(subdomains):
    groups = {}
    for sub in subdomains or []:
        key = _family_key(sub)
        groups.setdefault(key, []).append(sub)
    return [{"family": k, "count": len(v), "members": sorted(v)[:20]} for k, v in sorted(groups.items(), key=lambda kv: len(kv[1]), reverse=True)]


def cluster_certificate_sans(sans):
    groups = {}
    for san in sans or []:
        key = _family_key(san)
        groups.setdefault(key, []).append(san)
    return [{"family": k, "count": len(v), "sans": sorted(set(v))[:20]} for k, v in sorted(groups.items(), key=lambda kv: len(kv[1]), reverse=True)]


def historical_dns_diff_model(result):
    ...
    for ip, sources in all_ips.items():
        ...
        out.append({
            'ip': ip,
            'current_dns': 'current-dns' in srcs,
            'historical_only': bool(srcs & {'wayback', 'crtsh', 'urlscan.io', 'alienvault-otx'}) and 'current-dns' not in srcs,
            'sources': sorted(srcs),
        })
    out.sort(key=lambda x: (x['historical_only'], len(x['sources'])), reverse=True)
    return out[:50]


def build_candidate_comparison(result):
    rows = []
    for c in result.get('origin_candidates', []):
        rows.append({
            'ip': c['ip'],
            'role': c.get('role', ''),
            'discovery': c.get('discovery_score', 0),
            'origin': c.get('origin_score', 0),
            'confidence': c.get('confidence_score', 0),
            'verify': (c.get('verify') or {}).get('verdict', '-'),
            'sources': len(c.get('sources', [])),
            'subs': len(c.get('related_subdomains', [])),
            'cloud': c.get('cloud') or '-',
        })
    rows.sort(key=lambda x: (x['confidence'], x['origin'], x['subs']), reverse=True)
    return rows


def detect_environment_blocked(result):
    verify = result.get('verify_results', {}) or {}
    if not verify:
        return {'blocked': False, 'reason': 'verify mərhələsi işləməyib'}
    total = len(verify)
    nullish = 0
    timeouts = 0
    unreach = 0
    for data in verify.values():
        errs = data.get('errors', {}) or {}
        null_paths = 0
        for scheme in ('http', 'https'):
            for fp in (data.get('paths', {}) or {}).get(scheme, {}).values():
                if not fp.get('status'):
                    null_paths += 1
        if null_paths >= 2:
            nullish += 1
        timeouts += errs.get('timeout', 0) + errs.get('tls', 0)
        unreach += errs.get('network-unreachable', 0)
    blocked = total > 0 and nullish == total
    reason = 'environment blocked or unavailable' if blocked else 'ən azı bəzi probe cavabları alınıb'
    return {'blocked': blocked, 'reason': reason, 'nullish_hosts': nullish, 'total_hosts': total, 'timeouts': timeouts, 'unreachable': unreach}


def weight_source(src):
    if src in {'mx-record'}:
        return 0.5
    if src in {'spf-record'}:
        return 0.4
    if src.startswith('sub:'):
        return 2.5
    tier, w = source_trust(src)
    if tier == 'DIRECT':
        return float(w)
    if tier == 'VERIFIED_API':
        return max(1.0, w - 0.2)
    return max(0.5, w - 0.5)


def score_split(sources, cloud=None, verify=None, rdns_count=0, safe_audit=None, priority=0, bgp=None, ipapi=None, ip=None, shodan=None, related_subdomains=None):
    s = set(sources)
    discovery = sum(weight_source(x) for x in s)
    origin = 0.0
    negatives = []
    positives = []
    ...
    if has_sub:
        discovery += 2.5
        origin += 3.5
        positives.append('non-CF subdomain signal')
    if verify:
        base = max(0, float(verify.get('score', 0)))
        origin += base
        verdict = verify.get('verdict')
        if verdict == 'LIKELY_ORIGIN':
            origin += 8
            positives.append('strong multi-path verify')
        elif verdict == 'POSSIBLE':
            origin += 4
            positives.append('partial multi-path verify')
        elif verdict == 'LIKELY_BLOCKED_ORIGIN':
            origin += 2
            positives.append('blocked-origin behavior')
        elif verdict == 'EDGE_OR_PROXY':
            origin -= 2
            negatives.append('verify edge/proxy göstərir')
        if verify.get('matches', 0) >= 3:
            origin += 2
    if safe_audit:
        rs = safe_audit.get('risk_score', 0)
        if rs >= 8:
            discovery += 2
            origin += 1
            positives.append('safe exposure signal')
    if priority:
        discovery += min(4, priority)
        origin += min(3, priority)
    if rdns_count:
        penalty = min(6, rdns_count // 6)
        discovery -= penalty * 0.5
        origin -= penalty
        negatives.append('shared-host / reverse-DNS noise')

    # negative evidence
    if has_mail and not has_sub:
        origin -= 5
        discovery -= 0.5
        negatives.append('mail-only infra')
    org = ((bgp or {}).get('asn_name') or (ipapi or {}).get('org') or '').lower()
    if 'google' in org or (ip and str(ip).startswith('142.251.')):
        origin -= 6
        negatives.append('Google MX/CDN siqnalı')
    cpes = ((shodan or {}).get('cpes') or []) if shodan else []
    if any('cloudflare' in str(x).lower() for x in cpes):
        origin -= 6
        negatives.append('Cloudflare CPE')
    if ip and ':' in str(ip) and verify and (verify.get('errors', {}) or {}).get('network-unreachable', 0):
        origin -= 3
        negatives.append('unreachable IPv6')
    if cloud in EDGE_PROVIDERS:
        origin -= 8
        negatives.append('known edge/WAF provider')
    elif cloud in {'AWS', 'GCP', 'Azure', 'DigitalOcean', 'Vultr', 'Hetzner', 'OVH', 'Linode'}:
        origin += 1.5
        positives.append('hosting provider signal')
    if related_subdomains:
        subs = len(related_subdomains)
        if subs >= 2:
            origin += min(4, subs)
            positives.append('multiple related subdomains')

    discovery = round(max(0, discovery), 2)
    origin = round(max(-25, origin), 2)
    return discovery, origin, round(discovery + origin, 2)


def explain_ip(sources, verify=None, cloud=None, rdns_count=0, safe_audit=None, priority=0, bgp=None, ipapi=None, ip=None, shodan=None, related_subdomains=None):
    reasons = []
    if any(s.startswith('sub:') for s in sources):
        reasons.append('yuxarı: non-CF subdomain üzərindən görünür')
    if verify:
        reasons.append(f"verify: {verify.get('verdict','UNKNOWN')} / score={verify.get('score',0)} / matches={verify.get('matches',0)}")
    if priority:
        reasons.append(f'yuxarı: prioritet host izi ({priority})')
    if safe_audit and safe_audit.get('risk_score', 0):
        reasons.append(f'yuxarı: safe audit risk={safe_audit.get("risk_score",0)}')
    org = ((bgp or {}).get('asn_name') or (ipapi or {}).get('org') or '').lower()
    if ('mx-record' in sources or 'spf-record' in sources) and not any(s.startswith('sub:') for s in sources):
        reasons.append('aşağı: yalnız mail infrastrukturu izi')
    if 'google' in org or (ip and str(ip).startswith('142.251.')):
        reasons.append('aşağı: Google MX/CDN izi')
    cpes = ((shodan or {}).get('cpes') or []) if shodan else []
    if any('cloudflare' in str(x).lower() for x in cpes):
        reasons.append('aşağı: Cloudflare CPE izi')
    if rdns_count >= 10:
        reasons.append('aşağı: shared-host ehtimalı yüksəkdir')
    if cloud in EDGE_PROVIDERS:
        reasons.append(f'aşağı: edge provider ({cloud})')
    if ip and ':' in str(ip) and verify and (verify.get('errors', {}) or {}).get('network-unreachable', 0):
        reasons.append('aşağı: unreachable IPv6')
    if not reasons:
        reasons.append('neytral: güclü müsbət və ya mənfi siqnal azdır')
    return reasons


def build_text_summary(domain, posture, anomalies, origin_candidates, env=None):
    lines = [f'Hədəf: {domain}', f"Public risk: {posture.get('risk_level')} ({posture.get('risk_score')})", f'Anomaliya sayı: {len(anomalies)}']
    if env:
        lines.append(f"Mühit: {env.get('reason')}")
    preferred = None
    for ip, data in origin_candidates:
        if data.get('role') not in {'mail-related', 'cloudflare-edge'}:
            preferred = (ip, data)
            break
    if preferred:
        top = preferred[1]
...
def build_markdown_report(result):
    ...
    candidates = result.get('origin_candidates', [])
    anomalies = result.get('anomalies', [])
    comp = result.get('candidate_comparison', [])
    env = result.get('environment', {})
    lines = []
    lines += ['# CF-HUNTER v7.2.1 SAFE report', '', '## Executive summary', f"- Domain: `{meta.get('domain','')}`", f"- Elapsed: `{meta.get('elapsed_seconds','?')}s`", f"- Full mode: `{meta.get('full_mode', False)}`", f"- Environment: `{env.get('reason','unknown')}`", f"- Public posture: **{posture.get('risk_level','UNKNOWN')}** ({posture.get('risk_score',0)})", '']
    lines += ['## Analyst summary', f"- {meta.get('analysis_summary','-')}", '']
    lines += ['## Candidate comparison table', '', '| IP | Role | Conf | Disc | Orig | Verify | Subs | Cloud |', '|---|---:|---:|---:|---:|---|---:|---|']
    for row in comp[:15]:
        lines.append(f"| `{row['ip']}` | {row['role']} | {row['confidence']} | {row['discovery']} | {row['origin']} | {row['verify']} | {row['subs']} | {row['cloud']} |")
    if not comp:
        lines.append('| - | - | - | - | - | - | - | - |')
    lines += ['', '## Top 5 strongest candidates']
    for c in candidates[:5]:
        lines.append(f"- `{c['ip']}` | role={c['role']} | confidence={c.get('confidence_score',0)} | verify={(c.get('verify') or {}).get('verdict','-')}")
        for ex in c.get('explanation', [])[:6]:
            lines.append(f'  - {ex}')
    if not candidates:
        lines.append('- Tapılmadı')
    lines += ['', '## Weak candidates excluded and why']
    for row in comp[15:25]:
        lines.append(f"- `{row['ip']}` | conf={row['confidence']} | verify={row['verify']} | role={row['role']}")
    if len(comp) <= 15:
        lines.append('- Əlavə zəif namizəd yoxdur')
    lines += ['', '## Non-CF exposures grouped by type']
    for fam in result.get('subdomain_families', [])[:15]:
        lines.append(f"- `{fam['family']}` | count={fam['count']} | members={', '.join(fam['members'][:5])}")
    if not result.get('subdomain_families'):
        lines.append('- Tapılmadı')
    lines += ['', '## Public posture summary']
    hdrs = posture.get('security_headers', {})
    lines.append(f"- Missing security headers: {', '.join(hdrs.get('missing', [])) if hdrs.get('missing') else 'yoxdur'}")
    lines.append(f"- Interesting paths: {len(posture.get('interesting_paths', []))}")
    lines += ['', '## Infrastructure anomalies']
    for a in anomalies[:20]:
        lines.append(f"- `{a['subdomain']}` | providers={', '.join(a['providers'])} | {a['reason']}")
    if not anomalies:
        lines.append('- Tapılmadı')
    lines += ['', '## Timeline section']
    for item in result.get('historical_dns_diff', [])[:15]:
        lines.append(f"- `{item['ip']}` | historical_only={item['historical_only']} | current_dns={item['current_dns']} | sources={', '.join(item['sources'][:5])}")
    if not result.get('historical_dns_diff'):
        lines.append('- Timeline məlumatı yoxdur')
    lines += ['', '## Raw evidence section', '```json', json.dumps({'verify_results': result.get('verify_results', {}), 'source_health': result.get('source_health', {})}, ensure_ascii=False, indent=2)[:12000], '```']
    return "\n".join(lines)


def save_csv_report(result, csv_path):
    rows = result.get('candidate_comparison', [])
    if not rows:
        return
...
def build_adaptive_paths(full=False, baseline_blocked=False):
    base = list(FULL_VERIFY_PATHS if full else DEFAULT_VERIFY_PATHS)
    if baseline_blocked:
        for p in ['/index.html', '/home', '/status', '/robots.txt?view=1']:
            if p not in base:
                base.append(p)
    return base
    ...
    for scheme in ('http', 'https'):
        for path in paths:
            ...
            fp = {
                ...
                'body_hash': hashlib.sha256((body or '')[:8192].encode()).hexdigest()[:16],
                'header_keys': sorted(list(normalize_headers({k.lower(): v for k, v in (headers or {}).items()}).keys()))[:20],
                'location': ((headers or {}).get('Location', '') or (headers or {}).get('location', ''))[:120],
                'body_bucket': body_bucket(body or ''),
                'set_cookie': 'set-cookie' in {k.lower() for k in (headers or {})},
                'powered_by': ((headers or {}).get('X-Powered-By', '') or (headers or {}).get('x-powered-by', ''))[:80],
            }
            base[scheme][path] = fp
    # adaptive fallback
    all_null = all(not fp.get('status') for scheme in base.values() for fp in scheme.values())
    if all_null and not domain.startswith('www.'):
        fallback = f'www.{domain}'
        for scheme in ('http', 'https'):
            body, headers, status = http_get_with_headers(f"{scheme}://{fallback}/", timeout=8, cache=cache)
            if status:
                base[scheme]['/fallback-www'] = {
                    'status': status,
                    'server': (headers or {}).get('Server', '') or (headers or {}).get('server', ''),
                    'title': '',
                    'body_hash': hashlib.sha256((body or '')[:8192].encode()).hexdigest()[:16],
...
def verify_candidate_multi(ip, domain, baselines, paths=None):
    paths = paths or DEFAULT_VERIFY_PATHS
    verdict = {
        'ip': ip, 'score': 0, 'verdict': 'UNCONFIRMED', 'confidence_label': 'LOW', 'reasons': [],
        'paths': {'http': {}, 'https': {}}, 'matches': 0,
        'errors': {'timeout': 0, 'tls': 0, 'empty': 0, 'network-unreachable': 0, 'other': 0},
        'ok_count': 0,
    }
    baseline_blocked = all(not fp.get('status') for scheme in baselines.values() for fp in scheme.values())
    for scheme in ('http', 'https'):
        for path in paths:
            probe = http_probe_ip(ip, domain, scheme=scheme, path=path)
            fp = fingerprint_response(probe)
            ...
            if probe.get('squid_like'):
                verdict['score'] -= 3
                verdict['reasons'].append(f'{scheme}{path}: proxy-like')
            baseline_fp = (baselines.get(scheme) or {}).get(path, {})
            s, rs = compare_fingerprints(baseline_fp, fp)
            if s > 0:
                verdict['score'] += s
                verdict['matches'] += 1
                verdict['reasons'].extend([f'{scheme}{path}: {r}' for r in rs])
    if verdict['matches'] >= 4 and verdict['score'] >= 12:
        verdict['verdict'] = 'LIKELY_ORIGIN'
    elif verdict['matches'] >= 2 or verdict['score'] >= 6:
        verdict['verdict'] = 'POSSIBLE'
    elif verdict['ok_count'] == 0 and (verdict['errors'].get('timeout', 0) + verdict['errors'].get('tls', 0) >= 2 or baseline_blocked):
        verdict['verdict'] = 'LIKELY_BLOCKED_ORIGIN'
        verdict['reasons'].append('baseline və ya candidate probe bloklanmış görünür')
    else:
        verdict['verdict'] = 'EDGE_OR_PROXY'
    verdict['confidence_label'] = 'HIGH' if verdict['score'] >= 14 else 'MEDIUM' if verdict['score'] >= 7 else 'LOW'
    return verdict


def _print_explain_for_ip(result, ip):
    found = None
    for c in result.get('origin_candidates', []):
        if c.get('ip') == ip:
            found = c
            break
    if not found and ip in result.get('all_ips', {}):
        # synthesize
        sources = result['all_ips'][ip]
        enrich = (result.get('ip_enrichment', {}) or {}).get(ip, {})
        verify = (result.get('verify_results', {}) or {}).get(ip, {})
        found = {
            'ip': ip,
            'role': 'candidate',
            'sources': sources,
            ...
            'cloud': enrich.get('cloud'),
            'related_subdomains': [],
            'explanation': explain_ip(sources, verify=verify, cloud=enrich.get('cloud'), bgp=enrich.get('bgp'), ipapi=enrich.get('ipapi'), shodan=enrich.get('shodan'), ip=ip),
        }
    if found:
        print(f"\n{C.CYAN}{C.BOLD}[EXPLAIN]{C.RESET} {ip}")
        print(f"  Role: {found.get('role')}")
        print(f"  Verify: {(found.get('verify') or {}).get('verdict','-')}")
        for line in found.get('explanation', []):
            print(f"  - {line}")
    else:
        print(f"\n{C.YELLOW}[!] IP report içində tapılmadı: {ip}{C.RESET}")


async def _resolve_all_async(subs):
    loop = asyncio.get_running_loop()
    async def one(sub):
        try:
            infos = await loop.getaddrinfo(sub, None, proto=socket.IPPROTO_TCP)
            return sub, list({i[4][0] for i in infos})
        except Exception:
            return sub, []
    tasks = [one(s) for s in subs]
    results = await asyncio.gather(*tasks)
    return {sub: ips for sub, ips in results if ips}


def resolve_all(subs, workers=60):
    try:
        return asyncio.run(_resolve_all_async(subs))
    except Exception:
        out = {}
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as ex:
            for sub, ips in ex.map(_resolve_one, subs):
...
def analyze(domain, output_file=None, verbose=False, skip_github=False, skip_verify=False,
            verify_workers=10, resolve_workers=60, posture_workers=10, full=False,
            cache_enabled=True, cache_ttl=21600, resume=True, write_md=False,
            probe_timeout=6, probe_retries=2, quick=False, recon_only=False,
            posture_only=False, verify_only=False, top_n=50, export_csv=None,
            explain_ip_addr=None, save_state=False):
    if quick:
        skip_github = True
        full = False
        verify_workers = min(verify_workers, 8)
        resolve_workers = min(resolve_workers, 40)
        posture_workers = min(posture_workers, 6)
    if posture_only:
        skip_verify = True
        skip_github = True
        full = False
    ...
    result = _analyze_core(
        domain=domain, output_file=None, verbose=verbose, skip_github=skip_github,
        skip_verify=skip_verify, verify_workers=verify_workers, resolve_workers=resolve_workers,
        posture_workers=posture_workers, full=full, cache_enabled=cache_enabled,
        cache_ttl=cache_ttl, resume=resume, write_md=False, probe_timeout=probe_timeout,
        probe_retries=probe_retries,
    )
    result['environment'] = detect_environment_blocked(result)
    result['subdomain_families'] = cluster_subdomain_families(list((result.get('subdomains') or {}).get('resolved', {}).keys()))
    result['certificate_san_clusters'] = cluster_certificate_sans((result.get('tls') or {}).get('sans', []))
    result['historical_dns_diff'] = historical_dns_diff_model(result)
    result['candidate_comparison'] = build_candidate_comparison(result)
    result['meta']['tool'] = 'CF-HUNTER v7.2.1 SAFE'
    result['meta']['analysis_summary'] = build_text_summary(result['meta'].get('domain', domain), result.get('public_posture', {}), result.get('anomalies', []), [(c['ip'], c) for c in result.get('origin_candidates', [])], env=result['environment'])
    result['origin_candidates'] = result.get('origin_candidates', [])[:max(1, top_n)]
    ...
    if output_file:
        ...
        print(f"\n{C.GREEN}[+] JSON saxlandı: {output_file}{C.RESET}")
    if write_md:
        md_path = output_file.rsplit('.', 1)[0] + '.md' if output_file else f"{domain}_cfhunter_v7_2_1.md"
        Path(md_path).write_text(build_markdown_report(result), encoding='utf-8')
        print(f"{C.GREEN}[+] Markdown saxlandı: {md_path}{C.RESET}")
    if export_csv:
        save_csv_report(result, export_csv)
        print(f"{C.GREEN}[+] CSV saxlandı: {export_csv}{C.RESET}")
    if explain_ip_addr:
        _print_explain_for_ip(result, explain_ip_addr)
    if save_state and output_file:
        Path(output_file + '.state.json').write_text(json.dumps({'saved_at': time.time(), 'domain': domain, 'output': output_file}, ensure_ascii=False, indent=2), encoding='utf-8')
    return result


def main():
    banner()
    ap = argparse.ArgumentParser(description='CF-Hunter v7.2.1: adaptive, weighted, analyst-grade unified Cloudflare origin intelligence and posture auditor')
    ...
    ap.add_argument('--save-state', action='store_true', help='Əlavə sadə state faylı da saxla')
    ap.add_argument('--md-report', action='store_true', help='Markdown report da yarat')
    ap.add_argument('--export-csv', help='Candidate comparison üçün CSV çıxış yolu')
    ...
    if not re.match(r'^[a-z0-9][a-z0-9._-]+\.[a-z]{2,}$', domain):
        ...
        sys.exit(1)
    analyze(
        domain=domain, output_file=args.output, verbose=args.verbose, skip_github=args.no_github,
        skip_verify=args.skip_verify, verify_workers=max(1, min(args.verify_workers, 100)),
        resolve_workers=max(1, min(args.resolve_workers, 200)), posture_workers=max(1, min(args.posture_workers, 50)),
        full=args.full, cache_enabled=not args.no_cache, cache_ttl=max(60, args.cache_ttl),
        resume=not args.no_resume, write_md=args.md_report, probe_timeout=args.probe_timeout,
        probe_retries=args.probe_retries, quick=args.quick, recon_only=args.recon_only,
        posture_only=args.posture_only, verify_only=args.verify_only, top_n=args.top,
        export_csv=args.export_csv, explain_ip_addr=args.explain_ip_addr, save_state=args.save_state,
    )

def _main_old():
    banner()
    ap = argparse.ArgumentParser(description="CF-Hunter v7.2.1: safer and smarter unified Cloudflare origin intelligence, posture auditor and analyst report engine")
    ap.add_argument("domain", help="Hədəf domain")
    ap.add_argument("-o", "--output", help="JSON faylına yaz")
    ap.add_argument("-v", "--verbose", action="store_true", help="Ətraflı çıxış")
